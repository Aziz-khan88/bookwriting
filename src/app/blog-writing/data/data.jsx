//Banner
import BannerPoster from "media/services/article-writing/bannerImg.webp"

export const BannerData = {
    subtitle: "Top- ranked Article Writing company in US",
    title: "Bridging Success by Providing Professional Article Writing Services!",
    desc: "Our proficient and top-rated article writing services ensure your business gets the recognition it deserves!",
    BannerPoster: BannerPoster.src
}
//Servcies
export const InnerContentData = {
    subtitle: "Our Services",
    title: "High-Quality Article Writing Services To Expand Your Reach and Join The Big Leagues!",
    desc: "Expanding your reach with professional article writing services can be a great way to get your business in front of more potential customers. By having engaging and well-written content on your website, you can attract more visitors and keep them engaged with what you have to offer. Professional research article writing services can help you create content that is both informative and engaging, helping you to connect with more potential customers and grow your business.",
    innerServItem: [
        {
            title: "What is article writing services?",
            txt: "Article writing services are a great way to get high-quality, well-written articles for your website or blog. By outsourcing your writing needs to a professional service, you may rest assured that you'll be getting content that is both informative and engaging. Additionally, article writing services can help you build up your online presence by increasing your website's or blog's search engine ranking.",
        },
        {
            title: "How does an article writing service work?",
            txt: "Most high-quality article writing services work with a team of writers who create the content for their clients. The client will provide the topic or keywords that they want the content to be about, and the writers will create articles based on those topics.",
        },
        {
            title: "What are the benefits of using an article writing service?",
            txt: "There are several benefits to using our best article writing services. First, it can save you a lot of time. If you were to write the articles yourself, it would take up a lot of your time that could be used for other things. Second, it can help you get high-quality content. The writers who work for these services are professionals who know how to write good quality articles. Finally, it can help you build up your online presence. By having good quality content on your website or blog, you will attract more visitors, which can lead to more customers or clients.",
        },
        {
            title: "How do professional article writing services benefit businesses?",
            txt: "There are several ways that businesses can benefit from using a professional article writing service. First, it can help you save time. Second, it can help you get high-quality content. The writers who work for these services are professionals who know how to write good quality articles. Finally, it can help you build up your online presence.",
        },

    ]
}

// Main Content
import IMG01 from "media/services/article-writing/1.webp"
import IMG02 from "media/services/article-writing/2.webp"
import IMG03 from "media/services/article-writing/3.webp"

export const MainContentData = {
    subtitle: "Why Choose ",
    title: "Professional SEO Article Writing Services That Make an Impact",
    desc: "Our team of article writers has worked on countless projects from various niches.",
    MainContentItem: [
        {
            title: "Quality",
            txt: "At Book Writing Cube, we only work with experienced and professional writers who are experts in their field. This ensures that your articles will be of the highest quality and will provide accurate and up-to-date information.",
            Image: IMG01.src
        },
        {
            title: "Creativity",
            txt: "Our content development team is also highly creative, meaning they can come up with unique and engaging ways to present your information. This helps to make your articles stand out from the rest and gives readers a reason to keep coming back.",
            Image: IMG02.src
        },
        {
            title: "Plagiarism Free",
            txt: "When you are looking for SEO article writing services, it is important to find one that offers plagiarism-free content. This is something that Book Writing Cube takes pride in offering. All of their articles are written from scratch, so you can be sure that the content is 100% original.",
            Image: IMG03.src
        },

    ]
}


// Process
import ProcessImage from "media/services/article-writing/ProcessImage.webp"
export const ProcessData = {
    subtitle: "Our Process",
    title: "Efficient Article Writing Process Curated For Optimal Results",
    desc: "Book Writing Cube offers professional and custom article writing services to help businesses reach new heights of success! Here's a closer look at our streamlined process:",
    ProcessImage: ProcessImage.src
}


//Services
export const InnerServicesData = {
    //  column: "yes",
    subtitle: "Our Services Offer",
    title: "Why Our Spectacular Article Writing Services Are Ideal For Your Business",
    desc: "",
    innerServItem: [
        {
            title: (<>Compelling Content</>),
            txt: "We know how to write great articles that engage your audience and promote your brand.",
        },
        {
            title: (<>Experienced Writers</>),
            txt: "Our team of experienced writers can produce high-quality content quickly and efficiently.",
        },
        {
            title: (<>Wide Range of Services</>),
            txt: "We offer a wide range of quality article writing services, so you can find the perfect package for your needs.",
        },
        {
            title: (<>Customer Satisfaction</>),
            txt: "We're always happy to work with you to ensure that your articles are exactly what you need.",
        },
        {
            title: (<>Cost-Effective Prices</>),
            txt: "Our prices are highly competitive, and we offer various discounts for our valuable customers.",
        },
        {
            title: (<>Dedicated & Passionate</>),
            txt: "We're dedicated to providing excellent service from start to finish, and we're always available to answer any questions you might have.",
        },
    ]
}


//  Content Data
import BannerImage from "media/services/article-writing/Img01.webp"
export const ContentData = {
    title: "Get High-Quality and Effective Article Writing Services for Affordable Prices at Book Writing Cube!",
    desc: "Get in touch with one of our industry experts for more information about our online article writing services and pricing details by leaving your email or phone number below!",
    BannerImage: BannerImage.src,
}

